-- ULTIMATE Migration: Fix ALL missing columns across ALL tables
-- Date: 2024-01-XX
...
-- ========================================
-- Add isActive column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS isActive BOOLEAN DEFAULT true;
COMMENT ON COLUMN certificates.isActive IS 'Whether the certificate is active';

-- Add status column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'ISSUED';
COMMENT ON COLUMN certificates.status IS 'Status of the certificate (ISSUED, REVOKED, EXPIRED)';

-- Add expiresAt column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS expiresAt TIMESTAMP;
COMMENT ON COLUMN certificates.expiresAt IS 'When the certificate expires';

-- Add isRevoked column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS isRevoked BOOLEAN DEFAULT false;
COMMENT ON COLUMN certificates.isRevoked IS 'Whether the certificate has been revoked';

-- Add revokedAt column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS revokedAt TIMESTAMP;
COMMENT ON COLUMN certificates.revokedAt IS 'When the certificate was revoked';

-- Add revokedBy column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS revokedBy TEXT;
COMMENT ON COLUMN certificates.revokedBy IS 'User ID who revoked the certificate';

-- Add revokedReason column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS revokedReason TEXT;
COMMENT ON COLUMN certificates.revokedReason IS 'Reason for revoking the certificate';

-- Add pdfUrl column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS pdfUrl TEXT;
COMMENT ON COLUMN certificates.pdfUrl IS 'URL to download the certificate PDF';

-- Add pdfPath column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS pdfPath TEXT;
COMMENT ON COLUMN certificates.pdfPath IS 'File path to the certificate PDF';

-- Add createdAt column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
COMMENT ON COLUMN certificates.createdAt IS 'Creation timestamp';

-- Add updatedAt column if missing
ALTER TABLE certificates ADD COLUMN IF NOT EXISTS updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
COMMENT ON COLUMN certificates.updatedAt IS 'Last update timestamp';

-- ========================================
-- USERS TABLE (if missing columns)
-- ========================================
-- Add isActive column if missing
...
COMMENT ON COLUMN users.lastLoginAt IS 'When the user last logged in';

-- Add loginCount column if missing
ALTER TABLE users ADD COLUMN IF NOT EXISTS loginCount INTEGER DEFAULT 0;
COMMENT ON COLUMN users.loginCount IS 'Number of times user has logged in';
...
COMMENT ON COLUMN users.createdAt IS 'Creation timestamp';
...
SELECT 'certificates' as table_name, column_name 
FROM information_schema.columns 
